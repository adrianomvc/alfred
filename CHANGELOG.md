...
### Added
- Usage-cost adoption design: `docs/usage-cost-adoption.md` defines the next measurement path with DEVIN/Devin export as the primary corporate source, `ccusage` as a secondary local-CLI source, `ALFRED_RUN_ID` correlation, confidence labels, and a pilot checklist before automatic model-policy suggestions.
