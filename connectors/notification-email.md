# Connector: notification — email (D44)
...
- **Authorization:** durable (configured once) → sends transparently at configured points; records in `audit`. Asks only if it deviates from config (D41).
