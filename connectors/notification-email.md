...
- **Register (DEVIN CLI):** add to the project's `.devin/config.local.json` (gitignored):
  `{"mcpServers": {"alfred-email": {"command": "python", "args": ["<home>/.alfred/scripts/python/adapters/mcp-email-server.py"]}}}` — tools appear as `mcp__alfred-email__send_telemetry` etc.
