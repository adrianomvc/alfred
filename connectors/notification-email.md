...
The concrete channel is an MCP server: `scripts/python/adapters/mcp-email-server.py` (stdlib only, stdio JSON-RPC). Tools: `send_email(subject, body, to?, trigger?, attachments?)`, `send_demand_report(demand_path, to?, trigger?)` — reads `001-state.md` and auto-attaches the demand's metrics, audit, summary, and observability JSONL — and `email_status()`.
...
- **Register (Claude Code):** `claude mcp add alfred-email -- python <framework>/scripts/python/adapters/mcp-email-server.py` — any MCP host works.
