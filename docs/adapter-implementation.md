...
5. Run `scripts/validate-connectors.ps1` and add deterministic checks when the adapter has local behavior.
6. Record active status in the HUB/App `state`.
7. Run `scripts/validate-framework.ps1` for framework changes.
