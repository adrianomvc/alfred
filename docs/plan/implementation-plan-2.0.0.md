...
- [ ] W-SOLID.2 Migrar o restante para as camadas shared quando tocados: `generate-metrics-rollup.py`, `generate-metrics-insights.py`, `normalize-usage-cost.py` e os helpers de `scripts/metrics/observability.py` (`canonical_artifact`/`safe_path`/`classify_artifact` — hoje injetados no hook adapter; consolidar em `shared` e remover a duplicata do `domain/services/artifact_classifier.py`). Preencher os adapters ainda stub (`codex/*`, `devin/*`) com parsing real quando houver fonte/fixture.
