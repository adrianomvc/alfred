...
2. Collect metrics from `05-operation/011-observability-log.jsonl`: elapsed time, phase time, interactions, model/tokens/cost if available, retries, defects, blockers, and acceptance result.
   ...
   - If the host supports batch/flex/background execution, non-urgent rollups
...
Each step has concise JIT guidance in `sub-activities/` (`metrics-collection` / `baseline-drift-check` / `closure-summary` / `hub-sync` / `strategic-notification` / `followup-conversion` / `memory-distillation`) — load only the rungs the close triggers. These are sub-activities **inside Operation**, never new phases.
