...
**DEVIN CLI concrete map:** `swe-1-6-fast` (cheap) · `adaptive` (common medium work) · `opus` (strong default) · `gpt` (configured strong alternative). Adaptive is a router, not evidence of the effective tier: record the actual model when exposed, otherwise mark it `unconfirmed`. Design, SAFE, and critical operations select an explicit strong family. The DEVIN CLI switches models mid-session with `/model`. Devin exposes no live model to scripts (no env var, hook field, or session metadata), but it logs the resolved model at session start; `scripts/workflow/detect-host-model.py` reads it so the toolbar shows fact, not target.
