...
4. **JIT context load (anti-hypercontext):**
   ...
   - on resume → **rebuild from the `state`** and show the toolbar by running `scripts/python/workflow/render-toolbar.py` over the demand `001-state.md` (preferred). If the helper cannot run, load `presentation/toolbar-quick.md` before writing anything and emit only its text fallback shape from the same state fields; do not hand-draw a rich toolbar from memory.
