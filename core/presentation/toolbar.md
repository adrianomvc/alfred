...
- Visual richness follows `core/presentation/README.md`: `text` is the floor; `rich-cli` and `web` are optional, helper-rendered profiles over the same `state` (the model stays on the compact source — token economy).
