...
(sigla, id, lane, phase checklist, checkpoint, next step, model, cost). Borderless
text is the floor — nothing to misalign. Full spec (layers, profiles, rules):
...
- Progress = completed phases / 5; markers: `[x]` done · `[>]` current · `[ ]` pending.
