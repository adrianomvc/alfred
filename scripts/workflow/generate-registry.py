...
KNOWLEDGE_REQUIRED = {"name", "description", "scope", "trigger"}
...
def list_knowledge(root):
    return sorted(
        path for path in (root / "knowledge").glob("*.md")
        if path.name not in {"README.md", "knowledge.md"}
    )
...
def render_knowledge_registry(root):
    rows = []
    for path in list_knowledge(root):
        data = parse_frontmatter(path)
        require_keys(data, KNOWLEDGE_REQUIRED, rel(path, root))
        macro = scalar(data.get("macro", "")).strip().strip('"')
        rows.append(
            [
                f"`{data['name']}`",
                scalar(data["scope"]),
                ...
                f"[{Path(rel(path, root)).name}]({Path(rel(path, root)).name})",
            ]
        )
    ...
    lines = [
        "# Knowledge Registry",
        "",
        "> Generated by `scripts/workflow/generate-registry.py` from each policy's "
        "frontmatter. Do not edit the table by hand.",
        "",
        "Company knowledge is loaded by scope and event. This registry tells agents "
        "which policy file to load without reading the whole folder. Each policy "
        "declares its `scope`, retrieval `trigger`, and optional `macro` in "
        "frontmatter, mirroring how a host recalls knowledge on demand.",
        "",
        "## Registry",
    ...
    lines.extend([
        "",
        "## Rule",
        "Load only the policy whose trigger matches the active event. Sigla/HUB and "
        "demand knowledge may harden these policies, but must not weaken company "
        "policy without an explicit human exception in the audit.",
    ])
    return "\n".join(lines) + "\n"
...
def main():
    ...
    check_or_write(root / "knowledge" / "knowledge.md", render_knowledge_registry(root), args.check)
