...
## trigger
SAFE lane, or any unit whose logic has clear invariants (parsing, encoding,
serialization, math, idempotent/CDC operations, data reconciliation). Opt-in,
activated per demand.
...
## link
`skills/property-based-testing.md`

## sections to load
- properties: derive invariants from the functional design (round-trip,
  idempotency, commutativity, bounds, conservation).
...
- integration: language-specific tool comes from the active `lang-*` skill;
  this pack is the agnostic guidance (precedence D22/D36).
