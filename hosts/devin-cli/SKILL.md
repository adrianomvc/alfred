...
For Devin, automatic usage attribution requires a captured `devin-...` session id plus approved Session Insights/Consumption API export. Session or consumption totals update `001-state.md` for toolbar display; append JSONL only when the export provides interaction/request-granular usage. Interaction cost needs per-interaction cost from the Devin source or exact granular usage plus an approved rate card; do not allocate session ACU/USD totals into JSONL interactions. Without that source, keep `custo: nao coletado`; do not infer ACU/USD from wall time or terminal output. **Lighter self-serve path (no API):** `/usage` and `/context` are the only usage sources the DEVIN CLI exposes, and they are interactive REPL commands the human runs — Alfred cannot run them itself. At checkpoints, ask the human to run `/usage` (estimated session credit/ACU) and paste it; record it in `001-state.md` as `usage-cost: host usage command (/usage)`, `cost source: host_usage_command`, `cost confidence: estimated`, `cost granularity: session`, for toolbar display only (never appended to JSONL). Devin does not report USD on self-serve, so keep `cost usd` empty (`Custo USD: indisponível`) and show the ACU/credit estimate as consumo; never convert it to USD. This is the Devin analog of Claude Code `/cost` (`connectors/usage-cost.md`, `host_cost_command`). If a local CLI source is also present and supported by `ccusage`, it may update the toolbar state as secondary evidence, but Devin API remains the preferred ACU source.
