...
Load `~/.alfred/core/model-policy.md` only when selecting or switching the model. The DEVIN CLI switches models mid-session with `/model opus|sonnet|codex|adaptive`; map tiers as `swe-1-6-fast`→cheap, `sonnet`→medium, `opus`/`gpt`→strong, `adaptive`=router. Switching mid-demand invalidates the prompt cache (per model), so switch deliberately for a hard step, not per turn — see the cache note in `model-policy.md`. Record the model that actually ran — you do not have to ask: Devin logs it at session start under its `cli/logs` directory. Stamp it with `python ~/.alfred/scripts/workflow/detect-host-model.py --state-path <hub-demand>/001-state.md --host devin-cli`, and the toolbar reports the real model instead of an unconfirmed policy target.
...
For Devin, **token** attribution needs no API: the local session transcript (`cli/transcripts/`) carries exact per-step tokens — run `python ~/.alfred/scripts/metrics/attribute-usage-devin.py --state-path <hub-demand>/001-state.md --transcript-path <transcript>` after the session ends. **ACU/cost** is what still requires Session Insights or the Consumption API, since Devin publishes ACU only in the web UI; never convert tokens into ACU. Session or consumption totals update `001-state.md` for toolbar display; append JSONL only when the export provides interaction/request-granular usage. Interaction cost needs per-interaction cost from the Devin source or exact granular usage plus an approved rate card; do not allocate session ACU/USD totals into JSONL interactions. Without that source, keep `custo: nao coletado`; do not infer ACU/USD from wall time or terminal output. **Lighter self-serve path (no API):** `/usage` and `/context` are the only usage sources the DEVIN CLI exposes, and they are interactive REPL commands the human runs — Alfred cannot run them itself. At checkpoints, ask the human to run `/usage` and paste it. Its unit depends on the plan: **self-serve/Free reports quota used % + reset date** (e.g. `Quota used: 98% (remaining: 2%)`), paid reports credit/ACU. Record what it shows in `001-state.md` as `usage-cost: host usage command (/usage)`, `cost source: host_usage_command`, `cost confidence: estimated`, `cost granularity: session`, plus the reset date when present, for toolbar display only (never appended to JSONL). Devin does not report USD on self-serve, so keep `cost usd` empty (`Custo USD: indisponível`) and show whatever `/usage` reports (quota %, credit, or ACU) as consumo; never convert it to USD. When quota is nearly exhausted (e.g. >90% used), warn the human in one line. **Compute the session/demand delta with the calculator** (Alfred cannot run `/usage` itself): at demand start ask the human to run `/usage` once and record the baseline — `python ~/.alfred/scripts/metrics/session-cost.py -StatePath <hub-demand>/001-state.md -UsageText "<pasted>" --set-baseline`; at each checkpoint run it again without `--set-baseline` to get `usage demand consumed` (delta), `available %`, and — only if an approved rate card has `acu.usd_per_acu` — USD. The toolbar then shows `Ciclo: X/Y ACU · Z% disponível`, `Sessão: ~S ACU`, `Demanda: ~D ACU`. **If the demand has a budget** (`budget limit` in state), also run `python ~/.alfred/scripts/metrics/budget-monitor.py -StatePath <hub-demand>/001-state.md` at each checkpoint and follow `rules/common/budget-policy.md` (near → propose accelerating by skipping optional steps; exceeded → report delivered vs budget and pause). This is the Devin analog of Claude Code `/cost` (`connectors/usage-cost.md`, `host_cost_command`). If a local CLI source is also present and supported by `ccusage`, it may update the toolbar state as secondary evidence, but Devin API remains the preferred ACU source.
